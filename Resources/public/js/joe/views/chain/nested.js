joe.loader.load('utils/binder/entity_binder', function (EntityBinder) {
	var formDesc = [
		{ type: 'input', name: 'state', label: 'State', labelWidth: 140 },
		{ type: 'input', name: 'jobChain', label: 'Job', labelWidth: 140 },
		{ type: 'input', name: 'nextState', label: 'Next State', labelWidth: 140 },
		{ type: 'input', name: 'errorState', label: 'Error State', labelWidth: 140 },
		{ type: 'block', list: [
	...
	var ctrlsDesc = [
		{ type: 'button', name: 'apply', value: 'Apply Chain Node', width: 140 },
		{ type: 'button', name: 'newNode', value: 'New Chain Node', width: 140 },
		{ type: 'button', name: 'insertNode', value: 'Insert Chain Node', width: 140 }
	];

	var gridCtrlsDesc = [
		{ type: 'button', name: 'remove', value: 'Remove Nodes', width: 140 }
	];
	...
	function initGrid()
	{
		...
		cols.map(
			(col, i) => this.ctrls.grid.setColumnHidden(i, col.hidden ? col.hidden : false)
		);
	...
	function nodeToRow(isEnd, node)
	{
		return [
			node.state,
			isEnd ? 'Endnode' : 'Job Chain',
			isEnd ? '' : node.jobChain,
			isEnd ? '' : node.nextState,
			isEnd ? '' : node.errorState,
			''
		];
	}
	...
	function update()
	{
		var oldBinder = this.current.isEnd
					  ? this.binders.nodeEnd
					  : this.binders.nodeJobChain;
		
		var newBinder = this.current.newIsEnd
					  ? this.binders.nodeEnd
					  : this.binders.nodeJobChain;
	...
	function resetForm()
	{
		['state', 'jobChain', 'nextState', 'errorState'].map(
			name => this.ctrls.form.setItemValue(name, '')
		);
	...
	function currentToForm()
	{
		...
		if (!this.current.isEnd)
		{
			...
			['jobChain', 'nextState', 'errorState'].map(
				function (name) {
	...
	function refreshForm()
	{
		if (this.current)
		{
			...
			var f = this.current.newIsEnd
				  ? this.ctrls.form.disableItem
				  : this.ctrls.form.enableItem;
	...
	function refreshCtrls()
	{
		...
		else
		{
			...
			var canApply = this.current.newIsEnd
						 ? stateSet
						 : (stateSet && jobChainSet);
			
			if (canApply)
				this.ctrls.formCtrls.enableItem('apply');
			else
				this.ctrls.formCtrls.disableItem('apply');

			if (stateSet)
				this.ctrls.formCtrls.enableItem('insertNode');
			else
				this.ctrls.formCtrls.disableItem('insertNode');
		}
	}
	...
	var build = function (binder)
	{
		...
		view.setup = function (parent) {
			...
			this.binders.nodeJobChain = new EntityBinder('jobChainNodeJobChain',
														 binder,
														 'jobChainNodeJobChainCollection'
			);
			this.binders.nodeEnd = new EntityBinder('jobChainNodeEnd',
													binder,
													'jobChainNodeEndCollection'
			);
