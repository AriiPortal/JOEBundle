...
joe.loader.load('utils/binder/entity_binder', function (EntityBinder) {
	var rowDesc = [
		...
		{ name: 'exclusive', label: 'Exclusive', format: x => x ? 'yes' : 'no' }
	];

	var formDesc = [
		{ type: 'combo', name: 'lock', label: 'Lock', labelWidth: 150, width: 140 },
		{ type: 'newcolumn' },
		{ type: 'checkbox', name: 'exclusive', label: 'Exclusive', labelWidth: 150 }
	];

	var ctrlsDesc = [
		{ label: 'Apply Lock Use', action: applyLock },
		{ label: 'New Lock Use', action: newLock },
		{ label: 'Remove Lock Use', action: removeLock }
	];
