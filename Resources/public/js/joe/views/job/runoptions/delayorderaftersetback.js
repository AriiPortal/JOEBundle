...
joe.loader.load('templates/form_entity_adder', function (FormEntityAdder) {
	joe.loader.load('utils/binder/entity_binder', function (EntityBinder) {
		var rowDesc = [{ name: 'setbackCount', label: 'Set Back Count' }, { name: 'delay', label: 'Delay [hh:mm:]ss' }];

		var formDesc = [{ type: 'input', name: 'setbackCount', label: 'Set Back Count' }, { type: 'newcolumn' }, { type: 'checkbox', name: 'isMaximum', label: 'Max' }, { type: 'newcolumn' }, { type: 'input', name: 'hh', label: '', width: '20' }, { type: 'newcolumn' }, { type: 'input', name: 'mm', label: ':', width: '20' }, { type: 'newcolumn' }, { type: 'input', name: 'ss', label: ':', width: '20' }, { type: 'newcolumn' }, { type: 'label', label: '[hh:mm:]ss' }];
		...
		function dataToForm(form, data) {
			...
			var func = isMaximum ? form.checkItem : form.uncheckItem;
		...
		function formToData(form) {
			var delay = mkDelay(form.getItemValue('hh'), form.getItemValue('mm'), form.getItemValue('ss'));
