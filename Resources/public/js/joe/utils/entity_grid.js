var EntityGrid = (function() {
	...
	function EntityGrid(target, grid, rowDesc, binder) {
		...
		var header = this.rowDesc.map(x => x.label).join(', ');
		this.grid.setHeader(header);

		for (var i = 0; i < this.rowDesc.length; i++)
		{
			if (this.rowDesc[i].hasOwnProperty('visible') &&
				!this.rowDesc[i].visible)
			{
	...
	EntityGrid.prototype.selector = function() {
		...
		for (var i = 0; i < this.rowDesc.length; i++)
		{
			...
			if (desc.hasOwnProperty('select'))
				desc.select.map(x => selectField(x, selector));
			else
				selectField(desc.name, selector);
		}
		return selector;
	}
