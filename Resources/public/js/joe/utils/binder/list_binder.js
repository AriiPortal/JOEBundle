...
joe.loader.load('utils/binder/data_binder', function(DataBinder) {
	var ListBinder = (function() {
		...
		ListBinder.prototype.register = function(obj, callbacks,
												 selector, childSelector) {
			var setChildSelector = function (dep) {
				dep.childSelector = childSelector ? childSelector : {};
			}
			Binder.prototype.register.call(this, obj, callbacks,
										   selector, setChildSelector);
		}
			...
			if (this.data.hasOwnProperty(id))
			{
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState == 4)
					{
						...
						{
							console.error("Could not remove entity: "
										  + xhr.responseText);
						}
					}
				}.bind(this);
