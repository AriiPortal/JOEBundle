...
var Node = function () {
	...
	Node.prototype = {
		init: function init(lazy, label) {
			this.label = label;
			this.lazy = lazy;
			this.initialized = true;
		},
		addTo: function addTo(tree, parentId) {
			if (!this.initialized) {
     ...
     */
		...
		onOpen: function onOpen(state) {
			if (this.loadChildren == null) return false;

			if (!this.hasOwnProperty('loaded')) this.loaded = false;

			if (!this.loaded) {
				this.loadChildren(function () {
					this.loaded = true;

					if (this.tree.obj.openItem(this.obj.id) == undefined) this.shouldOpen = true;
				}.bind(this));
				return false;
			} else {
				return true;
