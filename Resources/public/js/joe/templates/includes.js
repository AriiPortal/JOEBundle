...
joe.loader.load('templates/form_entity_adder', function (FormEntityAdder) {
	joe.loader.load('utils/binder/entity_binder', function (EntityBinder) {
		var rowDesc = [{ name: 'file', label: 'File', fromData: function fromData(obj) {
				return obj.file ? obj.file : obj.liveFile;
			}
		}, { name: 'node', label: 'Node' }];

		var formDesc = [{ type: 'checkbox', name: 'isLive', label: 'from Hot Folder' }, { type: 'newcolumn' }, { type: 'input', name: 'file', label: '', inputWidth: 150 }, { type: 'newcolumn' }, { type: 'input', name: 'node', label: 'Node', inputWidth: 150 }];

		var ctrlsDesc = [{ label: 'Apply', action: applyFile }, { label: 'New', action: newFile }, { label: 'Remove', action: removeFile }];
		...
		function dataToForm(form, data) {
			...
			if (data) {
				file = data.file ? data.file : data.liveFile;
			...
			var func = isLive ? form.enableItem : form.disableItem;
