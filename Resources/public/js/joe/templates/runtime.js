(function() {
	...
	function dataToForm(data)
	{
		...
		if (data) {
			if (ptr.begin)
				begin = splitDelay(ptr.begin);

			if (ptr.end)
				end = splitDelay(ptr.end);

			this.form.setItemValue('timezone', ptr.timeZone);

			ptr.once
				? this.form.checkItem('let_once')
				: this.form.uncheckItem('let_once');

			ptr.letRun
				? this.form.checkItem('let_run')
				: this.form.uncheckItem('let_run');
	...
	function formToData() {
		...
		ptr.begin = mkDelay(this.form.getItemValue('begin_hour')
							, this.form.getItemValue('begin_min')
							, this.form.getItemValue('begin_sec')
						   );

		ptr.end = mkDelay(this.form.getItemValue('end_hour')
						  , this.form.getItemValue('end_min')
						  , this.form.getItemValue('end_sec')
						 );
	...
	function builder (binder) {
		...
		view.setup = function (parent) {
			var str = [
				{ type:"combo" , name:"timezone", label:"Timezone", inputWidth:140, labelWidth: 100, options: timezones },
				{ type:"fieldset" , name:"form_fieldset_1", label:"Time Slot", list:[
					{ type:"block" , name:"form_block_1", list:[
						{ type:"checkbox" , name:"let_run", labelWidth: 100, label:"Let Run"  },
						{ type:"newcolumn"   },
						{ type:"checkbox" , name:"let_once", labelWidth: 100, label:"Let Once"  }
					]  },
					{ type:"block" , name:"form_block_2", list:[
						{ type:"input" , name:"begin_hour", label:"Begin Time", labelWidth: 100, inputWidth:25  },
						{ type:"newcolumn"   },
						{ type:"input" , name:"begin_min", label:":", inputWidth:25  },
						{ type:"newcolumn"   },
						{ type:"input" , name:"begin_sec", label:":", inputWidth:25  },
						{ type:"newcolumn"   },
						{ type:"label" , name:"form_label_1", label:"hh:mm:ss"  }
					]  },
					{ type:"block" , name:"form_block_3", list:[
						{ type:"input" , name:"end_hour", label:"End Time", labelWidth: 100, labelAlign:"left", inputWidth:25  },
						{ type:"newcolumn"   },
						{ type:"input" , name:"end_min", label:":", inputWidth:25  },
						{ type:"newcolumn"   },
						{ type:"input" , name:"end_sec", label:":", inputWidth:25  },
						{ type:"newcolumn"   },
						{ type:"label" , name:"form_label_2", label:"hh:mm:ss"  }
					]  }
				]  },
				{ type:"fieldset" , name:"form_fieldset_2", label:"Schedule", list:[
					{ type:"combo" , name:"schedule", inputWidth:300 }
				]  },
			];
