...
joe.loader.load('templates/form_entity_adder', function (FormEntityAdder) {
	joe.loader.load('utils/binder/entity_binder', function (EntityBinder) {
		var rowDesc = [{ name: 'delayCount', label: 'Error Count' }, { name: 'Delay [hh:mm:]ss', label: 'delay' }];

		var formDesc = [{ type: 'input', name: 'delayCount', label: 'Error Count' }, { type: 'newcolumn' }, { type: 'radio', name: 'isDelayed', label: 'Stop', value: 'stop' }, { type: 'newcolumn' }, { type: 'radio', name: 'isDelayed', label: 'Delay', value: 'delay' }, { type: 'newcolumn' }, { type: 'input', name: 'hh', label: '', width: '20' }, { type: 'newcolumn' }, { type: 'input', name: 'mm', label: ':', width: '20' }, { type: 'newcolumn' }, { type: 'input', name: 'ss', label: ':', width: '20' }, { type: 'newcolumn' }, { type: 'label', label: '[hh:mm:]ss' }];

		var ctrlsDesc = [{ label: 'Apply Delay', action: applyDelay }, { label: 'New Delay', action: newDelay }, { label: 'Remove Delay', action: removeDelay }];
		...
		function refresh(form) {
			...
			['hh', 'mm', 'ss'].map(function (x) {
				return func.call(form, x);
			});
		}
		...
		function formToData(form) {
			...
			switch (form.getCheckedValue('isDelayed')) {
				case 'stop':
					...
					break;
				case 'delay':
					delay = mkDelay(form.getItemValue('hh'), form.getItemValue('mm'), form.getItemValue('ss'));
			}
